...
const ICONS: Record<string, ReactNode> = {
  ...
  mic: (
    <>
      {P("M12 1a3 3 0 00-3 3v8a3 3 0 006 0V4a3 3 0 00-3-3z")}
      {P("M19 10v2a7 7 0 01-14 0v-2M12 19v4M8 23h8")}
    </>
  ),
  ...
  volume: (
    <>
      {P("M11 5L6 9H2v6h4l5 4V5z")}
      {P("M19.07 4.93a10 10 0 010 14.14M15.54 8.46a5 5 0 010 7.07")}
    </>
  ),
  phone: P("M22 16.92v3a2 2 0 01-2.18 2 19.79 19.79 0 01-8.63-3.07 19.5 19.5 0 01-6-6 19.79 19.79 0 01-3.07-8.67A2 2 0 014.11 2h3a2 2 0 012 1.72 12.84 12.84 0 00.7 2.81 2 2 0 01-.45 2.11L8.09 9.91a16 16 0 006 6l1.27-1.27a2 2 0 012.11-.45 12.84 12.84 0 002.81.7A2 2 0 0122 16.92z"),
  video: (
    <>
      {P("M23 7l-7 5 7 5V7z")}
      {P("M1 5h15v14H1z")}
  ...
  pin: P("M16 12V4h1V2H7v2h1v8l-2 2v2h5.2v6h1.6v-6H18v-2l-2-2z"),
  calculator: (
    <>
      {P("M4 2h16v20H4z")}
      {P("M8 6h8M8 10h2M14 10h2M8 14h2M14 14h2M8 18h2M14 18h2")}
    </>
  ),
  camera: (
    <>
      {P("M23 19a2 2 0 01-2 2H3a2 2 0 01-2-2V8a2 2 0 012-2h4l2-3h6l2 3h4a2 2 0 012 2z")}
      {P("M12 17a4 4 0 100-8 4 4 0 000 8z")}
    </>
  ),
  download: P("M21 15v4a2 2 0 01-2 2H5a2 2 0 01-2-2v-4M7 10l5 5 5-5M12 15V3"),
  github: P("M12 2C6.477 2 2 6.484 2 12.017c0 4.425 2.865 8.18 6.839 9.504.5.092.682-.217.682-.483 0-.237-.008-.868-.013-1.703-2.782.605-3.369-1.343-3.369-1.343-.454-1.158-1.11-1.466-1.11-1.466-.908-.62.069-.608.069-.608 1.003.07 1.53 1.032 1.53 1.032.892 1.53 2.341 1.088 2.91.832.092-.647.35-1.088.636-1.338-2.22-.253-4.555-1.113-4.555-4.951 0-1.093.39-1.988 1.029-2.688-.103-.253-.446-1.272.098-2.65 0 0 .84-.27 2.75 1.026A9.564 9.564 0 0112 6.844c.85.004 1.705.115 2.504.337 1.909-1.296 2.747-1.027 2.747-1.027.546 1.379.202 2.398.1 2.651.64.7 1.028 1.595 1.028 2.688 0 3.848-2.339 4.695-4.566 4.943.359.309.678.92.678 1.855 0 1.338-.012 2.419-.012 2.747 0 .268.18.58.688.482A10.019 10.019 0 0022 12.017C22 6.484 17.522 2 12 2z"),
...
export function GitHubStars({ className = "" }: { className?: string }) {
  const [stars, setStars] = useState<number | null>(() => {
    try {
      const cached = sessionStorage.getItem("sher.gh_stars");
  ...
  useEffect(() => {
    ...
    fetch("https://api.github.com/repos/SudhirDevOps1/sher-messenger")
      .then((r) => r.json())
      .then((data: { stargazers_count?: number }) => {
        if (typeof data?.stargazers_count === "number") {
          setStars(data.stargazers_count);
          try {
            sessionStorage.setItem("sher.gh_stars", String(data.stargazers_count));
          } catch {}
        }
      })
      .catch(() => {});
  ...
  return (
    <a
      href="https://github.com/SudhirDevOps1/sher-messenger"
      target="_blank"
      rel="noopener noreferrer"
      className={`row group items-center gap-1.5 rounded-xl border border-[var(--line-strong)] bg-white/[0.06] hover:bg-white/[0.12] px-3 py-1.5 text-xs font-semibold text-[var(--ink)] shadow-sm backdrop-blur transition hover:border-[rgba(79,240,182,.4)] ${className}`}
      title="Star SudhirDevOps1/sher-messenger on GitHub"
    >
      <Icon name="github" size={14} className="transition group-hover:scale-110" />
      <span>GitHub</span>
      <span className="row items-center gap-0.5 rounded-md bg-black/40 px-1.5 py-0.5 text-[10.5px] font-mono text-[var(--acc)]">
