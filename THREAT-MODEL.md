...
| **Stolen unlocked device** | YES | sessionStorage key dies with the tab; vault is AES-GCM at rest; panic wipe | a screenshot taken before the wipe |
| **XSS / hostile browser extension** | YES | strict CSP, no inline handlers, no remote scripts, no `dangerouslySetInnerHTML` on user text | an injected script in your own origin reads plaintext pre-encryption — no web app can fully prevent this |
...
2. **Not screenshot-proof.** No client can prevent a photo of the screen. Blur-on-blur is a deterrent, not a control.
